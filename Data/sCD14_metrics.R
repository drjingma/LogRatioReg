...
res = foreach(
  ...
  lrl.metrics = c(
    mse = as.vector(crossprod(YTe.c - lrl.Yhat.test) / nrow(XTe)),
  ...
  lrl_SBP = matrix(0, nrow = p, ncol = 1)
  rownames(lrl_SBP) = colnames(XTr)
  lrl_SBP[lrl.betahat > 10e-8, ] = 1
  lrl_SBP[lrl.betahat < -10e-8, ] = -1
  
  saveRDS(
    lrl.metrics,
    paste0(output_dir, "/lrlasso2_metrics", file.end))
  
  saveRDS(
    lrl_SBP,
    paste0(output_dir, "/lrlasso2_sbp", file.end)
  )
