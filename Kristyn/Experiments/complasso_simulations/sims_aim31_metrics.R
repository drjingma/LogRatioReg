...
res = foreach(
  ...
  slr.Yhat.train = predict(
    fit_supervised$cv_exact, newx = computeBalances(X, btree), 
    type = "response", s = 'lambda.min') # used cv.glmnet fit
  slr.PE.train = as.vector(crossprod(Y - slr.Yhat.train) / n)
  # 1b. on test set #
  # get prediction error on test set
  slr.Yhat.test = predict(
    fit_supervised$cv_exact, newx = computeBalances(X.test, btree), 
    type = "response", s = 'lambda.min') # used cv.glmnet fit
  slr.PE.test = mean((slr.Yhat.test-Y.test)^2) # matches mse.pred
  # fit_supervised$mse.pred == slr.PE.test # TRUE!
  # 2. estimation accuracy (i.t.o. beta) #
  slr.thetahat = as.matrix(fit_supervised$beta.min) # used refit_exact
  slr.Uhat = getU(btree = btree)
  slr.betahat = getBeta(
    slr.thetahat, U = slr.Uhat) # used refit_exact
  ...
  slr.non0.thetahat = (slr.thetahat != 0) # used refit_exact
