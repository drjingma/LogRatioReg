...
# Date: 06/30/2021
# Notes: run line by line to compare to aim31
...
# }
beta <- c(rep(0,p-2),1,-1) # sparse theta and sparse beta -- aim31, line 41
...
# ) %dorng% {
  ...
  # define lambda like in aim31
  maxlam <- 2*max(abs(crossprod(log(X[1:n,]),Y)/n))
  ...
  # get sbp_supervised, like in aim31
  sbp_supervised <- sbp.fromHclust(btree)
  ba_supervised = computeBalances(X, btree = btree)
  # plot(btree)
  
  # choose lambda
  slr.lam.min.idx = which.min(slr$cvm)
  slr.lam.min = slr$lambda[slr.lam.min.idx] # matches aim31
  slr.a0 = slr$int[slr.lam.min.idx] # matches cv_exact in aim31, but not refit_exact in aim31 (they're not the same value)
  slr.thetahat = slr$bet[, slr.lam.min.idx]
  slr.Uhat = getU(btree = btree)
  slr.betahat = getBeta(slr.thetahat, U = slr.Uhat)
  
  # compare to a0 and beta from glmnet fit to lambda.min in aim31
  # note -- got the same lam.min, but different a0 somehow
  cv_exact = slr$cv.glmnet
  refit_exact <- glmnet(
    x=ba_supervised[1:n,],y=Y,family='gaussian',lambda=slr.lam.min)
  refit_exact$a0 # matches refit_exact$a0 in aim31, but refit_exact is used for prediction
  slr.a0 # matches cv_exact$a0, but cv_exact's beta is returned in run.glmnet
  ...
  slr.Yhat.test = refit_exact$a0 + 
    computeBalances(X.test, btree) %*% refit_exact$beta
