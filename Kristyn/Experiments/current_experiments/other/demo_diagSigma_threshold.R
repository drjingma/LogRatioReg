...
# slr$min.idx
# slr$theta0[[slr$min.idx[2]]][slr$min.idx[1]]
# slr$theta[[slr$min.idx[2]]][, slr$min.idx[1]]
# slr$num_covariates
# sum(slr$meets_threshold[[slr$min.idx[2]]])
plotSBP(slr$sbp_thresh[[slr$min.idx[2]]])

##############################################################################
# supervised log-ratios -- larger sigmasq (measurement/noise sd)
##############################################################################

sigma_eps_seq = seq(0, max(abs(beta)) / 2, length.out = 3)

slr_mats = list()
tree_plts = list()
slr_meets_thresh_mat = matrix(NA, nrow = p, ncol = length(sigma_eps_seq))
slr_num_covariates_vec = rep(NA, length(sigma_eps_seq))
slr_num_covariates_mat = matrix(NA, nrow = neta, ncol = length(sigma_eps_seq))
slr_eta_mat = matrix(NA, nrow = neta, ncol = length(sigma_eps_seq))

for(i in 1:length(sigma_eps_seq)){
  ##############################################################################
  # simulate data
  set.seed(seed + i)
  fake.data = simulateBalanceReg(
    mu = muW, Sigma = SigmaW, U = getU(sbp = SBP), n = 2 * n, theta = theta, 
    sigma.noise = sigma_eps_seq[i])
  colnames(fake.data$X) = names(beta)
  ...
  slr_meets_thresh_mat[, i] = slr$meets_threshold[[slr$min.idx[2]]]
  slr_num_covariates_vec[i] = slr$num_covariates[slr$min.idx[2]]
  slr_num_covariates_mat[, i] = slr$num_covariates
  slr_eta_mat[, i] = slr$eta
  tree_plts[[i]] = plotSBP(
    slr$sbp_thresh[[slr$min.idx[2]]], 
    title = paste0("sigma_eps = ", sigma_eps_seq[i]))
}
