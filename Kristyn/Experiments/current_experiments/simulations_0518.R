...
# Generate x ----
U1 <- runif(2*n,-0.5,0.5)
U2 <- runif(2*n,-0.5,0.5)
alpha1 <- 2*c(rep(1,r)/r,-rep(1,s)/s,rep(0,p-1-r-s))
...
xALR <- tcrossprod(as.matrix(U1), as.matrix(alpha1)) + 
  # tcrossprod(as.matrix(U2), as.matrix(alpha2)) +
  mvtnorm::rmvnorm(2*n,mean=rep(0,p-1),sigma = Sigma) * sigx
x <- alrinv(xALR)
...
## Generate y ----
theta <- 0.2
y <- theta * U1 + rnorm(2*n) * sigy
yb <- 1*(y>0)
# plot(cor(clrx,as.matrix(y)))

## Clustering with the population similarity ----
# popRho <- popGamma(c(alpha1,0),theta,sigy^2,sigx^2,U1) 
...
What.aitchison <- max(AitchisonVar[index,index]) - AitchisonVar[index,index]
pheatmap(What.aitchison,cluster_rows = F, cluster_cols = F,
         main='Heatmap of the Aitchison similarity')
Lhat.aitchison <- spectral.clustering(What.aitchison,zeta=0.1)
Lhat.aitchison
# Lhat.aitchison.eig <- eigen(Lhat.aitchison)
# plot(Lhat.aitchison.eig$values)
# df.aitchison <- data.frame('1'=Lhat.aitchison.eig$vectors[,1],
#                            '2'=Lhat.aitchison.eig$vectors[,2],
#                            '3'=Lhat.aitchison.eig$vectors[,3],
#                            '4'=Lhat.aitchison.eig$vectors[,4]) %>%
#   reshape2::melt() %>%
#   ggplot(aes(x=rep(1:9,4),y=value)) + geom_point() +
#   facet_wrap(~variable)
# print(df.aitchison)
# kmeans(Lhat.aitchison.eig$vectors[,1:2],centers=2,nstart=10)$cluster
