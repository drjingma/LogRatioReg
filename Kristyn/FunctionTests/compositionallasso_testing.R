...
source("functions/compositional_lasso.R")
...
# 
lambda = lambda_max
testCompLasso = fitCompositionalLASSOstandardized(
  stdZY$Xtilde, stdZY$Ytilde, lambda)
plot(testCompLasso$objectives ~ c(1:length(testCompLasso$objectives)), type = "l")
testCompLasso$beta
...
sum(abs(beta - betahat)) # 2.270
sum(abs(beta - tildebeta)) # 2.604
sum(betahat)

################################################################################

# test fitLASSOstandardized_seq for lambda = 0 (LS soln)
testLASSOstd_seq = fitCompositionalLASSOstandardized_seq(stdZY$Xtilde, stdZY$Ytilde)
...
# test fitLASSO
testLASSO = fitCompositionalLASSO(Z, Y)
