...
#   difference_alpha = 2 * eps
#   while(difference_beta >= eps | difference_alpha >= eps){
#     for (j in 1:p){
#       old_betaj = beta[j]
#       A_j = (1 / n) * Xtilde[ , j] %*% R + old_betaj # unconstrained LASSO softthresh arg, Y - X_{-j} beta_{-j}^k, partial residual
#       A2_j = A_j - mu * (sum(beta[-j]) + alpha) # constrained LASSO softthresh arg
...
#     objective = compositionalLasso(Xtilde, Ytilde, mu, beta, gamma = alpha * mu, lambda)
#     objectives = c(objectives, objective) # for testing ########################
#     difference = sqrt((old_objective - objective)^2)
#     old_objective = objective
#     # update alpha
#     alpha = alpha + sum(beta)
#     # differences in beta and alpha updates
#     difference_beta = sqrt(sum((old_beta - beta)^2))
#     difference_alpha = sqrt((old_alpha - alpha)^2)
#     old_beta = beta
#     old_alpha = alpha
#     print(paste0("difference in beta : ", difference_beta))
#     print(paste0("difference in alpha : ", difference_alpha))
...
################################################################################
# Match settings of subcompositional lasso, for comparisons #
################################################################################
...
# n_lambda - length of desired tuning parameter sequence, is only used when the tuning sequence is not supplied by the user
# eps - precision level for convergence assessment, default 0.0001
fitCompositionalLASSO.match = function(
  X ,Y, lambda_seq = NULL, n_lambda = 20, 
  eps = 0.0001, mu = 1, intercept = TRUE, scaling = TRUE){
  n = dim(X)[1]
  p = dim(X)[2]
  if(!is.null(lambda_seq)) n_lambda = length(lambda_seq)
  
  # if lambda_seq is NULL
  if (is.null(lambda_seq)){
    maxlam <- 2 * max(abs(crossprod(X, Y) / n))
    lambda_seq <- exp(seq(from = log(maxlam), to = log(1e-4), length.out = n_lambda))
  }
  
  # Center and scale like in subcompositional lasso implementation
  if (intercept){
    Y.mean <- mean(Y)
    Y <- Y - Y.mean
    X.mean <- colMeans(X)
    X <- scale(X, center = X.mean, scale = FALSE) # centering before scaling
  }
  if (scaling){
    X.sd <- apply(X, 2, sd)
    X <- scale(X, center = FALSE , scale = X.sd)
  }
  fitLASSO_seq = fitCompositionalLASSOstandardized_seq(
    X, Y, lambda_seq, n_lambda, eps, mu)
  # get beta_mat and beta0_vec
  beta_mat = fitLASSO_seq$beta_mat
  if (scaling){
    beta_mat = beta_mat / X.sd
  }
  beta0_vec = rep(0, n_lambda)
  if (intercept){
    beta0_vec <- Y.mean - as.vector(X.mean %*% beta_mat)
  }
  
  # Get lambda_seq for the output
  lambda_seq = fitLASSO_seq$lambda_seq
...
# k - number of folds for k-fold cross-validation, default is 5
# eps - precision level for convergence assessment, default 0.0001
# match.subcomp - scale X like subcompositional lasso, don't center y
cvCompositionalLASSO.match = function(
  X ,Y, lambda_seq = NULL, n_lambda = 20, k = 10, 
  eps = 0.0001, mu = 1, intercept = TRUE, scaling = TRUE){
  n = dim(X)[1]
  if(!is.null(lambda_seq)) n_lambda = length(lambda_seq)
  
  # Fit Lasso on original data using fitLASSO
  Lasso = fitCompositionalLASSO.match(
    X ,Y, lambda_seq, n_lambda, eps, mu, intercept, scaling)
  beta_mat = Lasso$beta_mat
  beta0_vec = Lasso$beta0_vec
  lambda_seq = Lasso$lambda_seq
  ...
  for (j in 1:k){
    ...
    # Calculate LASSO on that fold using fitLASSO
    Lasso_j = fitCompositionalLASSO.match(
      Xtrain, Ytrain, lambda_seq, n_lambda, eps, mu, intercept, scaling)
