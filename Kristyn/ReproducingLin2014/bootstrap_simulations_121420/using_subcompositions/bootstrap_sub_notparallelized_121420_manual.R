  ...
  for (j in 1:cv.K){
    ...
    for(m in 1:cv.n_lambda){
      ...
      if(all(!selected_variables)){ # if none selected
        ...
        # get the subcompositions for the train and test sets
        X.count.bs.sel = X.count.bs[, selected_variables, drop = FALSE]
        X.subcomp.bs = sweep(X.count.bs.sel, MARGIN = 1, STATS = rowSums(X.count.bs.sel), FUN = "/")
        log.X.subcomp.bs = log(X.subcomp.bs)
        # Training data
        Xtrain.sub = log.X.subcomp.bs[idfold != j, , drop = FALSE]
        # Test data
        Xtest = log.X.subcomp.bs[idfold == j, , drop = FALSE]
...
saveRDS(bs.results,
        file = paste0("Kristyn/ReproducingLin2014", 
                      ...
                      "/bootstraps_sub_121420_notparallel_manual.rds"))
