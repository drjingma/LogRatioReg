...
# trying with subcomposition
X.sel = X[, final.selected]
X.prop.sel = sweep(X.sel, MARGIN = 1, STATS = rowSums(X.sel), FUN = "/")
Xmat.sub = log(X.prop.sel)
all.equal(Xmat, Xmat.sub) # different!
par(mfrow = c(1, 1))
betahat0.paper.sub = as.numeric(mean(y) - betahat.paper %*% colMeans(Xmat.sub))
all.equal(colMeans(Xmat), colMeans(Xmat.sub)) # different!
all.equal(crossprod(Xmat), crossprod(Xmat.sub)) # different!
all.equal(betahat0.paper, betahat0.paper.sub) # same!?
yhat.paper.sub = betahat0.paper.sub + Xmat.sub %*% betahat.paper
all.equal(yhat.paper, yhat.paper.sub) # same!?
all.equal(Xmat.sub %*% betahat.paper, Xmat %*% betahat.paper) # same ....
# why? X \hat{\beta} = X (X'X)^{-1} X' y = P_X y
all.equal(Xmat %*% solve(crossprod(Xmat), crossprod(Xmat, y)), 
          Xmat.sub %*% solve(crossprod(Xmat.sub), crossprod(Xmat.sub, y))
) # different...
# what about constrained betabar?
# betabar = betahat - (X'X)^(-1) 1 [1' (X'X)^(-1) 1]^(-1) 1' betahat

xyrange.sub = range(y, yhat.paper.sub)
plot(main = "Using subcomposition", xlab = "Observed BMI", ylab = "Fitted BMI",
     x = y, y = yhat.paper.sub, col = 1, xlim = xyrange.sub, ylim = xyrange.sub)
abline(0, 1, lty = 2)
...
# calculate by hand
# beta-bar = beta-hat - (X'X)^(-1) 1 [1' (X'X)^(-1) 1]^(-1) (1' beta-hat)
beta.hat = solve(crossprod(Xmat.sub), crossprod(Xmat.sub, y))
# check that it matches lm()
beta.hat.lm = coefficients(lm(y ~ -1 + Xmat.sub))
all.equal(as.numeric(beta.hat), as.numeric(beta.hat.lm)) # they match, as expected
# compute beta.bar, constrained fit
Q = as.matrix(rep(1, dim(Xmat.sub)[2]))
beta.bar = beta.hat - solve(crossprod(Xmat.sub), Q) %*% 
  solve(crossprod(Q, solve(crossprod(Xmat.sub), Q)), crossprod(Q, beta.hat))
# check function output
beta.bar2 = clm(Xmat.sub, y, Q)
all.equal(beta.bar, beta.bar2) # they match :D

# fit to the original data
Q = as.matrix(rep(1, dim(Xmat.sub)[2]))
stdXY = standardize(Xmat.sub, y, center = FALSE, scale = FALSE)
# fit betabar
betabar.orig = clm(Xmat.sub, y, Q)
# we can ignore the warning,  we just need this as input to backStandardize()
# get betabar0
betabar.bstd.orig = backStandardize(stdXY, betabar.orig, scale = FALSE)
# calculat yhat from fit
yhat.orig = betabar.bstd.orig$betahat0 + Xmat.sub %*% betabar.bstd.orig$betahat
# plot observed vs fitted values
plot(main = "No centering/scaling", xlab = "Observed BMI", ylab = "Fitted BMI",
     x = y, y = yhat.paper, col = 2,
     xlim = xyrange, ylim = xyrange)
...
# transform Xmat.sub by doing scale = TRUE
stdXY = standardize(Xmat.sub, y, center = FALSE, scale = TRUE)
all.equal(y, stdXY$Ytilde)
all.equal(Xmat.sub, stdXY$Xtilde)
# fit betabar
betabar = clm(stdXY$Xtilde, stdXY$Ytilde, Q)
# we can ignore the warning,  we just need this as input to backStandardize()
...
betabar.bstd = backStandardize(stdXY, betabar, scale = FALSE)
# calculate yhat, 
yhat = betabar.bstd$betahat0 + Xmat.sub %*% betabar.bstd$betahat
# plot observed vs fitted values
plot(main = "Scaling", xlab = "Observed BMI", ylab = "Fitted BMI",
     x = y, y = yhat.paper, col = 2,
     xlim = xyrange, ylim = xyrange)
...
# transform Xmat.sub by doing center = TRUE
stdXY = standardize(Xmat.sub, y, center = TRUE, scale = FALSE)
all.equal(y, stdXY$Ytilde)
all.equal(Xmat.sub, stdXY$Xtilde)
# fit betabar
betabar = clm(stdXY$Xtilde, stdXY$Ytilde, Q)
...
# transform Xmat.sub by doing scale = TRUE
stdXY = standardize(Xmat.sub, y, center = TRUE, scale = TRUE)
all.equal(y, stdXY$Ytilde)
all.equal(Xmat.sub, stdXY$Xtilde)
# fit betabar
betabar = clm(stdXY$Xtilde, stdXY$Ytilde, Q)
