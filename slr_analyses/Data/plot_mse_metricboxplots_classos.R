...
for(i in (1:numSplits)){
  ...
  # clr-lasso
  clrl_tmp = t(data.frame(readRDS(paste0(
    output_dir, "/clrlasso_metrics",
    file.end0, "_sim", i, ".rds"
  ))))
  rownames(clrl_tmp) = NULL
  clrlasso_list[[i]] = data.table::data.table(clrl_tmp)
}
...
data.gg = data.gg %>% 
  mutate(
    Method = factor(
      Method, 
      levels = c(
        "classo-auc", "classo-orig", "classo", "clr-lasso"
      )
    )
  ) 
